...
const DashboardGallery = () => {
  ...
  const handleCountryChange = (e: any, index: number) => {
    setArtists((art: any) => {
      ...
      const item = {
        ...art[index],
        Country: JSON.parse(e.target.value)?.name,
        flag: JSON.parse(e.target.value)?.flag,
      };
      copy[index] = item;
      return copy;
    });
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="grid md:grid-cols-2 gap-10">
        {artists.map((item, index) => (
          <div
            className="grid md:grid-cols-2 gap-6 border rounded-md p-5"
            key={index + 445544554}
          >
            ...
            <div>
              <input
                type="checkbox"
                onChange={(e: any) => handleisDefault(e, index)}
      ...
      <div className="my-2">
        <input
          type="checkbox"
          onChange={(e: any) => setisHomeSlider(e.target.checked)}
          checked={isHomeSlider}
          name="isDefault"
          id="isDefault"
          className="h-[15px] w-[15px] mt-4"
        />{" "}
        &nbsp; <span>Add this to home slider</span>
      </div>
